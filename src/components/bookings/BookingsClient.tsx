            ...
            <form onSubmit={handleCreateBooking} className="space-y-4">
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <label className="text-xs text-zinc-400 font-bold block">Attach Catalog Services</label>
                  <select
                    value={serviceFilter}
                    onChange={e => setServiceFilter(e.target.value as any)}
                    className="bg-zinc-950 border border-zinc-800 rounded-md px-2 py-1 text-xs text-white outline-none focus:border-violet-500"
                  >
                ...
                <div className="space-y-2 max-h-36 overflow-y-auto p-2 bg-zinc-950 border border-zinc-850 rounded-xl">
                  {services
                    .filter((s: any) => serviceFilter === 'ALL' || s.executionType === serviceFilter)
                    .map((s: any) => (
